...
function showCardCount() {
    ...
    let playerCards = playerHands[0].length;
    let computerCards = playerHands[1].length;
...
async function handleTurn(player) {
    ...
    // If not face rule, check for slap
    else {
        await sleep(1000);
        eventFlag = true;
        if (pile.length != 0) {
...
document.getElementById('your-player-area').addEventListener('click', () => {
    if (currentPlayer === 0 && gameRunning == 1 && eventFlag) {
        handleTurn(currentPlayer);
    }
});
...
socket.on('playerAssigned', (number) => {
    ...
    // Set up event listeners based on player assignment
    if (playerNumber === 1) {
        document.addEventListener('keydown', handlePlayerOneControls);
    } else if (playerNumber === 2) {
        document.addEventListener('keydown', handlePlayerTwoControls);
    }
});